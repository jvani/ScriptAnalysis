...
if __name__ == "__main__":
    script = Script("./data/biglebowski.txt")
    nnames = [
        ...
        ["YOUNG WOMAN", "BUNNY"],
        ["MAUDE'S VOICE", "MAUDE"],
        ["FEMALE VOICE ON MACHINE", "MAUDE"],
        ["DUDE '", "DUDE"],
    ]

    for charname, nname in nnames:
        script.dialogue["Character"] = script.dialogue.Character.replace(
            charname, nname
        )

    (
        script.dialogue.groupby("Character")
        .agg(compound=("compound", "mean"), lines=("Character", "size"))
        .sort_values(by="lines", ascending=False)
        .to_csv("./data/biglebowski_character_sentiment.csv")
    )
    ...
    script.dialogue.where(script.dialogue.Character.isin(characters)).dropna().to_csv(
        "./data/biglebowski_main_character_sentiment.csv", index=False
    )
